...
public class UserService {
	...
	private void validateUserIdAndPassword(String userid, String userpw) {
	    ...
	    if (userid.length() < USERID_MIN_LENGTH || userid.length() > USERID_MAX_LENGTH) {
	        throw new IllegalArgumentException("아이디는 " + USERID_MIN_LENGTH + "자 이상 " + USERID_MAX_LENGTH + "자 이하이어야 합니다.");
	    }
	    ...
	    if (userpw.length() < USERPW_MIN_LENGTH || userpw.length() > USERPW_MAX_LENGTH) {
	        throw new IllegalArgumentException("비밀번호는 " + USERPW_MIN_LENGTH + "자 이상 " + USERPW_MAX_LENGTH + "자 이하이어야 합니다.");
	    }
	}
	...
	public void updateUser(String username, UpdateUserRequest updateRequest){
		// 사용자 정보 가져오기
		System.out.println("Service의 username : " + username);
		User user = userRepo.findByusername(username).orElseThrow(() -> new RuntimeException("유저를 찾을 수 없습니다"));
	    ...
	    if (userName != null && !userName.trim().isEmpty()) {
	        if (userName.length() < USERNAME_MIN_LENGTH || userName.length() > USERNAME_MAX_LENGTH) {
	            throw new IllegalArgumentException("닉네임은 " + USERNAME_MIN_LENGTH + "자 이상 " + USERNAME_MAX_LENGTH + "자 이하이어야 합니다.");
	        }
